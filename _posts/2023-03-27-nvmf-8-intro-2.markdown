...
async fn handle_client(mut stream_tokio: TcpStream, active_queues: &mut HashMap<u16, IoQueue>) -> Result<(), Box<dyn Error>> {
    ...
    match opcode {
        ...
        0x0A => {
            ...
            let n = NamespaceAttachment::new(stream, active_queues, qid, cid).await?;
            n.handle().await?;
        }
        0x0B => {
            // Keep Alive command
            let k = KeepAlive::new(stream, active_queues, qid, cid).await?;
            k.handle().await?;
        }
        0x0C => {
            // Directive Send command
            let d = DirectiveSend::new(stream, active_queues, qid, cid).await?;
            d.handle().await?;
        }
        0x0D => {
            // Directive Receive command
            let d = DirectiveReceive::new(stream, active_queues, qid, cid).await?;
            d.handle().await?;
        }
        0x0E => {
            // Virtualization Management command
            let v = VirtualizationManagement::new(stream, active_queues, qid, cid).await?;
            v.handle().await?;
        }
        0x0F => {
            // NVMe-MI Send command
            let n = NvmeMiSend::new(stream, active_queues, qid, cid).await?;
            n.handle().await?;
        }
        0x10 => {
            // NVMe-MI Receive command
            let n = NvmeMiReceive::new(stream, active_queues, qid, cid).await?;
            n.handle().await?;
        }
        0x11 => {
            // Doorbell Buffer Config command
            let d = DoorbellBufferConfig::new(stream, active_queues, qid, cid).await?;
            d.handle().await?;
        }
        0x7F => {
            // Format NVM command
            let f = FormatNvm::new(stream, active_queues, qid, cid).await?;
            f.handle().await?;
        }
        0x80 => {
            // Security Send command
            let s = SecuritySend::new(stream, active_queues, qid, cid).await?;
            s.handle().await?;
        }
        0x81 => {
            // Security Receive command
            let s = SecurityReceive::new(stream, active_queues, qid, cid).await?;
            s.handle().await?;
        }
        0x82 => {
            // Sanitize command
            let s = Sanitize::new(stream, active_queues, qid, cid).await?;
            s.handle().await?;
        }
        0x83 => {
            // Get Log Page command
            let g = GetLogPage::new(stream, active_queues, qid, cid).await?;
            g.handle().await?;
        }
        0x84 => {
            // Get LBA Status command
            let g = GetLbaStatus::new(stream, active_queues, qid, cid).await?;
            g.handle().await?;
        }
        0x85 => {
            // Read command
            let r = Read::new(stream, active_queues, qid, cid).await?;
            r.handle().await?;
        }
        0x86 => {
            // Write command
            let w = Write::new(stream, active_queues, qid, cid).await?;
            w.handle().await?;
        }
        0x87 => {
            // Write Uncorrectable command
            let w = WriteUncorrectable::new(stream, active_queues, qid, cid).await?;
            w.handle().await?;
        }
        0x88 => {
            // Compare command
            let c = Compare::new(stream, active_queues, qid, cid).await?;
            c.handle().await?;
        }
        0x89 => {
            // Write Zeroes command
            let w = WriteZeroes::new(stream, active_queues, qid, cid).await?;
            w.handle().await?;
        }
        0x8A => {
            // Dataset Management command
            let d = DatasetManagement::new(stream, active_queues, qid, cid).await?;
            d.handle().await?;
        }
        0x8B => {
            // Reservation Register command
            let r = ReservationRegister::new(stream, active_queues, qid, cid).await?;
            r.handle().await?;
        }
        0x8C => {
            // Reservation Report command
            let r = ReservationReport::new(stream, active_queues, qid, cid).await?;
            r.handle().await?;
        }
        0x8D => {
            // Reservation Acquire command
            let r = ReservationAcquire::new(stream, active_queues, qid, cid).await?;
            r.handle().await?;
        }
        0x8E => {
            // Reservation Release command
            let r = ReservationRelease::new(stream, active_queues, qid, cid).await?;
            r.handle().await?;
        }
        0x90 => {
            // Timestamp command
            let t = Timestamp::new(stream, active_queues, qid, cid).await?;
            t.handle().await?;
        }
...
impl Queue {
    fn new(qid: u16, qsize: u16) -> Queue {
        Queue { qid, qsize }
    }
}
...
struct CreateConnection {
    stream: TcpStream,
    active_queues: HashMap<u16, Queue>,
    qid: u16,
    cid: u16,
}

impl CreateConnection {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> CreateConnection {
        CreateConnection {
            stream,
            active_queues,
            qid,
...
impl NamespaceManagement {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> NamespaceManagement {
        NamespaceManagement {
            stream,
            active_queues,
            qid,
...
impl DatasetManagement {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> DatasetManagement {
        DatasetManagement {
            stream,
            active_queues,
            qid,
...
impl ReservationRegister {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> ReservationRegister {
        ReservationRegister {
            stream,
            active_queues,
            qid,
...
impl ReservationReport {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> ReservationReport {
        ReservationReport {
            stream,
            active_queues,
            qid,
...
impl ReservationAcquire {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> ReservationAcquire {
        ReservationAcquire {
            stream,
            active_queues,
            qid,
...
impl ReservationRelease {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> ReservationRelease {
        ReservationRelease {
            stream,
            active_queues,
            qid,
...
impl Timestamp {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> Timestamp {
        Timestamp {
            stream,
            active_queues,
            qid,
...
impl Identify {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> Identify {
        Identify {
            stream,
            active_queues,
            qid,
...
impl Abort {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> Abort {
        Abort {
            stream,
            active_queues,
            qid,
...
impl SetFeatures {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> SetFeatures {
        SetFeatures {
            stream,
            active_queues,
            qid,
...
impl GetFeatures {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> GetFeatures {
        GetFeatures {
            stream,
            active_queues,
            qid,
...
impl AsynchronousEventRequest {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> AsynchronousEventRequest {
...
impl AsynchronousEvent {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> AsynchronousEvent {
        AsynchronousEvent {
            stream,
            active_queues,
            qid,
...
impl NamespaceAttachment {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> NamespaceAttachment {
        NamespaceAttachment {
            stream,
            active_queues,
            qid,
...
impl KeepAlive {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> KeepAlive {
        KeepAlive {
            stream,
            active_queues,
            qid,
...
impl DirectiveSend {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> DirectiveSend {
        DirectiveSend {
            stream,
            active_queues,
            qid,
...
impl DirectiveReceive {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> DirectiveReceive {
        DirectiveReceive {
            stream,
            active_queues,
            qid,
...
impl VirtualizationManagement {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> VirtualizationManagement {
...
impl NvmeMiSend {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> NvmeMiSend {
        NvmeMiSend {
            stream,
            active_queues,
            qid,
...
impl NvmeMiReceive {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> NvmeMiReceive {
        NvmeMiReceive {
            stream,
            active_queues,
            qid,
...
impl DoorbellBufferConfig {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> DoorbellBufferConfig {
...
impl FormatNvm {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> FormatNvm {
        FormatNvm {
            stream,
            active_queues,
            qid,
...
impl SecuritySend {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> SecuritySend {
        SecuritySend {
            stream,
            active_queues,
            qid,
...
impl SecurityReceive {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> SecurityReceive {
        SecurityReceive {
            stream,
            active_queues,
            qid,
...
impl Sanitize {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> Sanitize {
        Sanitize {
            stream,
            active_queues,
            qid,
...
impl GetLogPage {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> GetLogPage {
        GetLogPage {
            stream,
            active_queues,
            qid,
...
impl GetLbaStatus {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> GetLbaStatus {
        GetLbaStatus {
            stream,
            active_queues,
            qid,
...
impl FirmwareCommit {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> FirmwareCommit {
        FirmwareCommit {
            stream,
            active_queues,
            qid,
...
impl FirmwareImageDownload {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> FirmwareImageDownload {
...
impl WriteUncorrectable {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> WriteUncorrectable {
        WriteUncorrectable {
            stream,
            active_queues,
            qid,
...
impl Compare {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> Compare {
        Compare {
            stream,
            active_queues,
            qid,
...
impl WriteZeroes {
    fn new(stream: TcpStream, active_queues: HashMap<u16, Queue>, qid: u16, cid: u16) -> WriteZeroes {
        WriteZeroes {
            stream,
            active_queues,
            qid,
